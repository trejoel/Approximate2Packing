...
void Alg_SG_Capas_Contiguas() {
	...
	if (cplex.getStatus() == IloAlgorithm::Optimal) {
		...
		//
		/*if (NumSG < 2) {
			env.out() << "\n Values = " << vals[10] << "," << vals[20] << "," << vals[50] << endl;
		}*/
	...
	for (unsigned int j = 0; j < NumNodosSG[NumeSG]; j++) {
		...
		for (int i : vSG[NumeSG][j].liga)
		{
			if (edgSG[NumeSG][i].vertex1 != j) {
				//cond += x[edgSG[NumeSG][i].vertex1];
				//costoSG += valoresSG[NumeSG][j];
...
int main()
{
	...
	GMLread("D:/semestres/sem2-2020/GraphJoel/Graph Files (ene 26)/Outerplanar500_1.gml");
