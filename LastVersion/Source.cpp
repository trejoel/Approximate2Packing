...
void Subdivide()
{
	...
	{
		//printf("%d(%d),",v[i].id, v[i].level);
		entra = 0;
		//si es el primer SG, entonces
		j1 = 0;
...
void TestConstraintsSG(int N) {
	...
	/*if(N==1)
		for (int i = 0; i < NumNodosSG[N]; i++)
		{
			printf("(i=%d)vSG[%d][%d]=%d,",i,N, i, vSG[N][i].id);
		}
		printf("\n");*/
		/*for (int i = 0; i < NumEdgesSG[N]; i++)
		{
			printf("(i=%d)edgSG[%d][%d]=<%d,%d>,", i, N, i, edgSG[N][i].vertex1, edgSG[N][i].vertex2);
		}
		printf("\n");*/
}
//***********************************************
void TestConstraintsSGv1(int i, int SumaNodosSG) {
	...
	for (unsigned int j = 0; j < NumNodosSG[i]; j++)
		...
		//if(i<3 && j<10)
		//printf("===== vSG[%d][%d].id=%d,  con valor=%d equivale a v[%d].id=%d\n", i, j, vSG[i][j].id, valoresSG[i][j], vSG[i][j].id, v[vSG[i][j].id].id);
		TotalValores = valoresSG[i][j]; //el valor 0, 1 del nodo analizado
		for (int p : vSG[i][h - SumaNodosSG].liga) {
			//printf("<%d(%d), %d(%d)>, ", edgSG[i][p].vertex1, vSG[i][edgSG[i][p].vertex1].level, edgSG[i][p].vertex2, vSG[i][edgSG[i][p].vertex2].level);
			if (edgSG[i][p].vertex1 != h - SumaNodosSG) {
				//printf("vecino  %d es vSG[%d][%d]=%d,                           valoresSG[%d][%d]=%d\n", edgSG[i][p].vertex1, i, edgSG[i][p].vertex1, vSG[i][edgSG[i][p].vertex1].id, i, edgSG[i][p].vertex1, valoresSG[i][edgSG[i][p].vertex1]);
				TotalValores += valoresSG[i][edgSG[i][p].vertex1];
			}
			else {
				//printf("vecino  %d es vSG[%d][%d]=%d,                            valoresSG[%d][%d]=%d\n", edgSG[i][p].vertex2, i, edgSG[i][p].vertex2, vSG[i][edgSG[i][p].vertex2].id, i, edgSG[i][p].vertex2, valoresSG[i][edgSG[i][p].vertex2]);
				TotalValores += valoresSG[i][edgSG[i][p].vertex2];
			}
		}//for p
		//printf("TotalValores=%d \n", TotalValores);
...
void CapasContiguas()
{
	...
	//printf("cambiaron k=%d nodos en SG=0 \n", k);
	printf("SGs intermedios \n");
	//los SGs intermedios hay que cambiar valores de los nodos de la primera y ultima capas
...
//***********************************************
void GMLwriteToExcel(char* p_filename)
{
	...
	{
		fout << i << "\t" << NivelBegin[i] << "," << NivelEnd[i] << "  " << NumNodosSG[i] << "\t" << NumEdgesSG[i] << "\t" << costo_old[i] << "\t" << tiempo[i] << "\t\t" << costo[i] << endl;
	}
	fout << " " << "\t" << " " << "\t" << " " << "\t" << " " << "\t" << "Costo" << "\t" << " " << "\t" << "Costo con sol. conflictos" << endl;
	fout << " " << "\t" << " " << "\t" << " " << "\t" << " " << "\t" << CostoGlobal_old << "\t" << " " << "\t\t" << CostoGlobal << endl;
...
int main()
{
	...
	int summing = 0; //checando el numero de nodos
	...
	//GMLread("D:/semestres/sem2-2020/GraphJoel/Graph Files (ene 26)/Outerplanar500_2.gml");
	printf("Termina de cargar archivo G, presiona una tecla\n");
	//MaxNumSG = 10; //numero de SGs
	h = 50;
	MaxNumSG = MaxNivel / h;
	...
	//NivelLeader = 0; //inicia para el coloreado, no se si aun me sirve
	//Creacion de subGrafos
	//unsigned int h = 50; //numero de niveles o capas por SG
	...
	if (sobran > 0) {
		...
		/*printf("Hay que ajustar los arreglos del i=%d a MaxNumSG-1=%d \n", i, MaxNumSG - 1);
	...
	//Improvement . . . . . . . . . . . . . . . . . . . . . . . . . . . . BEGIN
	printf("\n Improvement para SG con capas eliminadas . . . . . . . . . . . . . . . . \n");
	printf("Pulsa una tecla...\n"); _getch();

	for (unsigned int i = 0; i < MaxNumSG; i++)
		for (unsigned int j = 0; j < NumNodosSG[i]; j++) {
			A = vSG[i][j].id; //para cada nodo en el SG
			B = valoresSG[i][j];
			//v[A].color[0] = B; v[A].color[2] = X;
			valores[A] = B;
			//if(i<2 && j>NumNodosSG[i]-10)
			//printf("v[%d].color[0]=%d \n", A, B);
		}

	unsigned int costoEnd = 0, costoBegin = 0;
	int costoEndBegin = 0;
	//---------------------------------------------------------------------------------------------------------------------------------
	//en lugar de 3 seria MaxNumSG-1
	for (unsigned int NumeroSG = 0; NumeroSG < MaxNumSG - 1; NumeroSG++) {
		//checando NivelEnd[ ]
		costoEnd = 0; costoBegin = 0;
		printf("SG[%d]\n", NumeroSG);
		/*for (unsigned int i = 0; i < NumNodosSG[NumeroSG]; i++)
		{
			if(i> NumNodosSG[NumeroSG]-20)
			printf("vSG[%d][%d].id=%d \n", NumeroSG, i, vSG[NumeroSG][i].id);
		}*/
		printf("Grafo General G, NivelEnd[%d] - 1=%d con %d nodos\n", NumeroSG, NivelEnd[NumeroSG] - 1, PorNivel[NivelEnd[NumeroSG] - 1].size());
		//for (int i = 0; i < PorNivel[NivelEnd[NumeroSG] - 1	].size(); i++) 
		printf("Nivel END - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -\n");
		int i = 0;
		while (i < PorNivel[NivelEnd[NumeroSG] - 1].size()) {
			//printf("v[  %d  ].id=%d -- ", i, PorNivel[NivelEnd[NumeroSG] - 1][i], v[PorNivel[NivelEnd[NumeroSG] - 1][i]].id);
			//printf("v[%d].id=%d ", v[PorNivel[NivelEnd[NumeroSG] - 1][i]].id, v[v[PorNivel[NivelEnd[NumeroSG] - 1][i]].id].id);
			//printf("con %d vecinos  ", v[PorNivel[NivelEnd[NumeroSG] - 1][i]].liga.size());
			if (checa_rojo_End(NumeroSG, i))
			{
				//printf("\n==rojo==\n"); 
				valores[v[PorNivel[NivelEnd[NumeroSG] - 1][i]].id] = 1;
				valores[v[PorNivel[NivelEnd[NumeroSG] - 1][i]].id + 1] = 0;
				valores[v[PorNivel[NivelEnd[NumeroSG] - 1][i]].id + 2] = 0;
				i = i + 3;
				costoEnd++;
			}
			else
			{
				//printf("\n==azul==\n"); 
				valores[v[PorNivel[NivelEnd[NumeroSG] - 1][i]].id] = 0;
				i++;
			}
			//printf("\n");
			//i++;
		...
		printf("\n");
		printf("Grafo General G, NivelBegin[%d] - 1=%d con %d nodos\n", NumeroSG + 1, NivelBegin[NumeroSG + 1] - 1, PorNivel[NivelBegin[NumeroSG + 1] - 1].size());
		printf("Nivel BEGIN - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -\n");
		i = 0;
		while (i < PorNivel[NivelBegin[NumeroSG + 1] - 1].size()) {
			//printf("v[  %d  ].id=%d -- ", i, PorNivel[NivelBegin[NumeroSG+1] - 1][i], v[PorNivel[NivelBegin[NumeroSG+1] - 1][i]].id);
			//printf("v[%d].id=%d ", v[PorNivel[NivelBegin[NumeroSG+1] - 1][i]].id, v[v[PorNivel[NivelBegin[NumeroSG+1] - 1][i]].id].id);
			//printf("con %d vecinos  ", v[PorNivel[NivelBegin[NumeroSG+1] - 1][i]].liga.size());
			if (checa_rojo_Begin(NumeroSG + 1, i))
			{
				//printf("\n==rojo==\n"); 
				valores[v[PorNivel[NivelBegin[NumeroSG + 1] - 1][i]].id] = 1;
				valores[v[PorNivel[NivelBegin[NumeroSG + 1] - 1][i]].id + 1] = 0;
				valores[v[PorNivel[NivelBegin[NumeroSG + 1] - 1][i]].id + 2] = 0;
				i = i + 3;
				costoBegin++;
			}
			else
			{
				//printf("\n==azul==\n"); 
				valores[v[PorNivel[NivelBegin[NumeroSG + 1] - 1][i]].id] = 0;
				i++;
			}
			//printf("\n");
			//i++;
		...
		printf("\n");
		costoEndBegin = costoEndBegin + costoEnd + costoBegin;
		printf("costo[%d] = %d, costoEnd=%d, costoBegin=%d, ", NumeroSG, costo[NumeroSG], costoEnd, costoBegin);
		if (NumeroSG == 0) {
			printf("Improvement = %d \n", costo[NumeroSG] + costoEnd);
		}
		else if (NumeroSG == MaxNumSG - 1) {
			printf("Improvement = %d \n", costo[NumeroSG] + costoBegin);
		}
		else {
			printf("Improvement = %d \n", costo[NumeroSG] + costoBegin + costoEnd);
		}

		printf("Pulsa una tecla para terminar NumeroSG = %d que ha aumentado su costo de %d a %d\n", NumeroSG, CostoGlobal, CostoGlobal + costoEndBegin);
		//_getch();
	}
	//---------------------------------------------------------------------------------------------------------------------------------
